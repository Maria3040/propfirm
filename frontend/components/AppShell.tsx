...
function Icon({ name }: { name: string }) {
  const common = { width: 22, height: 22, fill: 'currentColor', viewBox: '0 0 256 256' as const };
  switch (name) {
    case 'accounts':
      return (
        <svg {...common}>
...
function BrandMark() {
  return (
    <svg viewBox="0 0 45 49" fill="currentColor" aria-label="PropFirm" className="app-brand-mark">
      <g fillRule="evenodd">
        <path
          fillRule="nonzero"
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="app-shell">
      <div className="app-banner" role="region" aria-label="Announcements">
        <div className="app-banner-inner">
          <span aria-label="Announcement: New here? Get 20% OFF your first challenge with code HELLO. Excludes 100K accounts.">
      ...
      <aside className="app-sidebar" aria-label="App sidebar">
        <div className="app-sidebar-top">
          <Link href="/accounts" className="app-sidebar-brand" aria-label="Go to Dashboard">
            <BrandMark />
          </Link>
            ...
            {NAV.map((item) => {
              ...
              return (
                <Link
                  ...
                  title={item.label}
                  className={`app-sidebar-btn${active ? ' active' : ''}`}
        ...
        <div className="app-sidebar-bottom">
          <Link
            ...
            aria-label="Settings"
            title="Settings"
            className={`app-sidebar-btn${pathname.startsWith('/settings') ? ' active' : ''}`}
          ...
          {hasSession ? (
            <button type="button" className="app-sidebar-btn" aria-label="Log out" onClick={logout}>
              <Icon name="logout" />
            </button>
          ) : (
            <button
      ...
      <BasketDrawer
        ...
        onRemove={(key) => persist(basket.filter((i) => i.key !== key))}
      />
    </div>
  );
}
